...
export default function buildHierarchicalDataProvider(Private, Notifier) {
  ...
  return function (vis, resp) {
    ...
    let aggData = resp.aggregations[firstAgg.id] || resp.aggregations['special_special_' + firstAgg.id][firstAgg.id];
    ...
    let rows = _.map(extractBuckets(aggData, firstAgg), function (bucket) {
      ...
      let split = buildSplit(agg, metric, bucket[agg.id] || resp.aggregations['special_special_' + firstAgg.id][firstAgg.id]);
