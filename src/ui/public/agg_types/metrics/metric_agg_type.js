...
export default function MetricAggTypeProvider(Private) {
  ...
  MetricAggType.prototype.getValue = function (agg, bucket) {
    ...
    return (bucket[agg.id] || bucket['special_' + agg.id][agg.id] || bucket['special_special_' + agg.id][agg.id]) && (bucket[agg.id] || bucket['special_' + agg.id] || bucket['special_special_' + agg.id][agg.id]).value;
  };
